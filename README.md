...
```bash
sudo apt-get install git rake curl command-not-found tmux python-pip python3-pip remmina unrar
pip install absl-py python-ptrace --user
pip3 install absl-py python-ptrace --user
```
...
Also make sure a powerline font is selected in the profile.
...
It may be necessary to re-compile YCM:
